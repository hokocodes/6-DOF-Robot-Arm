...
void loop() {
  ...
  Serial.println("Twisted wrist");
    ...
    // move wrist joint to drop object
  pwm.setPWM(3, 0, 450);
  Serial.println("Twisted wrist joint");
  ...
  //  MOVE ELBOW TO DROP OBJECT
  for (uint16_t i = SERVOMAX; i > 425; i--){
    pwm.setPWM(2, 0, i);
    delay(25);
  }
  delay(1000);
  ...
  Serial.println("Twist base 2nd time");
    ...
    // MOVE SHOULDER JOINT
  for (uint16_t i = SERVOMAX; i > 450; i--){
    pwm.setPWM(1, 0, i);
    delay(25);
  }
  delay(1000);
  Serial.println("Twist shoulder joint");
